...
public class DeletePlaylist extends JDialog implements ActionListener {
	...
	public DeletePlaylist(MusicManager m) throws Exception {
    	mManager = m;
    	setTitle(Global.getLOCInstance().getLocalizedString("Sidebar.DeletePlaylist"));
        
        mButtonPanel = new JPanel();
        final JPanel panel = new JPanel(new BorderLayout());
        
        mOkButton = new JButton(Global.getLOCInstance().getLocalizedString("Button.DeleteSelected"));
        ...
        mNameList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if(mOkButton == event.getSource()){
            int selected[] = mNameList.getSelectedIndices();

            for(int i = 0; i < selected.length; i++) {
        		String name = mNames[selected[i]];
				try {
					int pid = Global.getSRVInstance().PlaylistService().GetPlaylistIdByName(name);
					Global.getSRVInstance().PlaylistService().deletePlaylist(pid);
					...
					e.printStackTrace();
					JOptionPane.showMessageDialog(this, Global.getLOCInstance().getLocalizedString("Error.CannotDelete"));
				}
        	}
